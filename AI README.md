...
series is currently five patches, which is about as small as it gets while still
...
bootstrap, copies in our extensions, renames the product in the built-in strings
that name it on screen, emits the licenses, and repairs two things the build gets
wrong for this deployment. `homepage.mjs` builds the landing page. Finally two
gates run: `check-endpoints.mjs` refuses to let a Microsoft endpoint sneak back
in, and `check-licenses.mjs` refuses to ship an extension that has not stated its
terms.

Output lands in `dist/`, in three parts that go to three different places. See
...
| Path                     | Purpose                                                                                            |
| ------------------------ | -------------------------------------------------------------------------------------------------- |
| `vscode.pin`             | Upstream release tag to build from (currently 1.138.0)                                             |
| `LICENSE`                | RuntimeCode's own MIT license                                                                      |
| `THIRD_PARTY_NOTICES.md` | RuntimeCode's additions; upstream `LICENSE.txt` + `ThirdPartyNotices.txt` appended at build        |
...
| `extensions/lua-wasmoon/` | The first runtime pack: Lua 5.4 via wasmoon, vendored with digests in `pack.json`                |
...
Three licenses, drawn along folder boundaries so that the boundaries are
checkable rather than asserted.

**RuntimeCode is MIT, and only MIT.** The bootstrap, the overlay, the patch
series and everything under `scripts/` are MIT, the same as Code - OSS upstream.
There is no copyleft in the core and nothing here is intended to put any there.

**Each extension carries its own license, in its own folder.** Every directory
under `extensions/` states an SPDX id in its `package.json` and ships a `LICENSE`
beside its code, and both travel into `dist/app/rc-extensions/<id>/`. The point
is not the two MIT extensions that exist today; it is the runtime packs in
[RUNTIMES.md](RUNTIMES.md), several of which are GPL. The web extension host
loads an extension by reading one file and calling `new Function(...)`
(`extHostExtensionService.ts:87`), so extensions are aggregated with the
workbench rather than linked into it, and a GPL extension can sit beside an MIT
editor without either license reaching the other. A GPL or AGPL extension must
also declare `"runtimecode": { "source": ... }`, naming an upstream revision or
`"in-tree"` when the shipped folder is the whole source.

**`homepage/` is AGPL-3.0, and nothing else is.** It builds to `dist/homepage/`,
a separate artifact deployed to a separate place. It covers the landing page's
markup, stylesheet and artwork. It does not cover the editor, the extensions, or
anything written with them.

`scripts/check-licenses.mjs` enforces all three: it fails the build on an
extension that has not stated its terms, on a license file that did not survive
the copy into `dist/`, and on strong-copyleft license text appearing under
`dist/app/` anywhere outside `rc-extensions/`. `staticify.mjs` runs the same
check before it moves anything, so the failure arrives in a second rather than
after a full package step.
...
Upstream is not always buildable, and two patches exist only for that. Both are
pure compile fixes with no runtime effect, and both should be dropped the moment
upstream fixes them.

`patches/0001` covers a defect present on `main` and on every tag we have built:
`sessionChangesEditor.ts` overrides two `protected` members without repeating the
keyword, which widens them to public, and the mangler rejects that outright.

`patches/0005` covers one that arrived with 1.138.0. Three tests in
`codexProviderConfiguration.test.ts` construct a `TestConfigurationClient` whose
callbacks read `client.readCount` off the binding being initialised; TypeScript
cannot infer through the cycle and fails with six `implicitly has type 'any'`
errors. It is a node-side test and reaches nothing in the web bundle, but
`compile-build-without-mangling` typechecks all of `src/`, so it stops the build
like anything else. An explicit type annotation is the whole fix.

A related trap: those errors surface only after `npm ci`. Before it, the same tag
fails earlier and differently, with type errors in `copilotAgent.ts` against a
`@vscode/copilot-api` version that belongs to the previous tag. `prepare.mjs`
prints a note about this; `upgrade.mjs` reinstalls for you. Running `prepare.mjs`
and `build.mjs` by hand across a tag bump does not.
...
| `dist/homepage/`  | wherever the site lives                                                      | no landing page; the editor is unaffected           |

The first two are the editor and are the ones that must move together. The
homepage is independent: it is a different license and a different deploy, and
nothing in the editor reads it.
