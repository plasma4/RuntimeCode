...
if (!target) {
	console.error('usage: node scripts/check-deploy.mjs <url of the deployed RuntimeCode folder>');
	process.exit(1);
}
...
function runtimeFsRoot(url) {
	...
	return index === -1 ? undefined : new URL(url.pathname.slice(0, index + 1), url.origin);
}
...
if (root) {
	...
	console.log(`[check-deploy] SKIP app          ${base} is a RuntimeFS virtual path, not fetchable from outside a browser`);
} else {
	const app = await head(new URL('index.html', base));
	if (app.ok) {
		console.log(`[check-deploy] OK   app          ${new URL('index.html', base)}`);
	} else {
		problems.push(`The RuntimeCode folder is not reachable at ${base} (HTTP ${app.status}).`);
	}
}

if (!root) {
	console.log('[check-deploy] SKIP host-root   no /n/ segment, so this is a standalone deployment');
	console.log('[check-deploy] standalone builds use the in-folder service worker and need no host-root file');
} else {
...
if (!existsSync(path.join(APP_OUT, 'index.html'))) {
	console.log('[check-deploy] note: no local build in dist/, so nothing was compared against it');
}
