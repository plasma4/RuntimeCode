...
import { assertVscodeCheckout, deepMerge, RC_ROOT, run, VSCODE_ROOT } from './lib.mjs';
...
function checkoutPin() {
	...
	try {
		current = execFileSync('git', ['describe', '--tags', '--exact-match'], {
			cwd: VSCODE_ROOT, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore']
		}).trim();
	} catch {
...
function mergeProduct() {
	...
	console.log(`[prepare] merged product.overlay.json -> product.json (${merged.nameLong})`);
}
...
function applyPatches() {
	...
	for (const patch of patches) {
		...
		run('git', ['apply', '--3way', path.join(patchDir, patch)], { cwd: VSCODE_ROOT });
	}
}
