...
import { matchesGlob, RC_ROOT, DIST } from "./lib.mjs";
...
function isAllowed(pattern, file) {
  return allowlist.some(
    (entry) =>
      ...
      entry.files.some((glob) => matchesGlob(glob, file)),
  );
}
  ...
  for (const pattern of PATTERNS) {
    ...
    if (isAllowed(pattern, file)) {
      ...
      unexplained.push({
        ...
        pattern,
        context: content
          .slice(Math.max(0, index - 70), index + 70)
          .replace(/\s+/g, " "),
      });
    }
  }
...
if (unexplained.length > 0) {
  ...
  if (unexplained.length > 30) {
    console.error(`  ...and ${unexplained.length - 30} more`);
  }
  console.error(
