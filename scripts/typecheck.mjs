...
const PROJECTS = [path.join(RC_ROOT, 'extensions', 'runtimefs', 'jsconfig.json')];

assertVscodeCheckout();

const source = path.join(VSCODE_ROOT, 'src', 'vscode-dts', 'vscode.d.ts');
if (!existsSync(source)) {
	throw new Error(`No vscode.d.ts at ${source}. Is ${VSCODE_ROOT} a full checkout?`);
}
...
if (!existsSync(tsc)) {
	throw new Error(`No TypeScript at ${tsc}. Run npm ci in ${VSCODE_ROOT} first.`);
}
